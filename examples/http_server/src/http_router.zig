...
const HttpResponseObject = struct {
    body: [16384]u8, // Fixed buffer for response body
    body_len: usize,
    status: u16,
    std: c.zend_object,
    ...
    pub fn setBody(self: *HttpResponseObject, body: phpzx.PhpString) void {
        // Copy the string data immediately to avoid dangling pointer issues
        const copy_len = @min(body.len, 16384);
        @memcpy(self.body[0..copy_len], body.ptr[0..copy_len]);
        self.body_len = copy_len;
    }
...
const HttpRouterObject = struct {
    ...
    pub fn dispatch(self: *HttpRouterObject, request: *HttpRequestObject, response: *HttpResponseObject) c.zend_long {
        const path_slice = request.path;

        for (self.*.routes.items) |*route| {
            const route_path = route.path[0..route.path_len];

            // Try to match the route (with or without parameters)
            var params: [8]c.zval = undefined;
            var param_count: u32 = 0;

            if (matchRoute(route_path, path_slice, &params, &param_count)) {
                // Found matching route - call the handler
                var retval: c.zval = undefined;

                // Make a complete copy of fci and fci_cache to avoid concurrent modification
                var fci_copy = route.handler_fci;
                var fci_cache_copy = route.handler_fci_cache;

                // Initialize the return value
                retval.u1.type_info = @intFromEnum(phpzx.PhpType.Null);

                // Set up parameters for the closure call (request, response, + route params)
                var call_params: [2]c.zval = undefined;

                // Create zvals for request and response objects
                const request_obj = &request.std;
                const response_obj = &response.std;

                call_params[0].value.obj = request_obj;
                call_params[0].u1.type_info = @intFromEnum(phpzx.PhpType.Object);

                call_params[1].value.obj = response_obj;
                call_params[1].u1.type_info = @intFromEnum(phpzx.PhpType.Object);

                // Modify the copy, not the original
                fci_copy.retval = &retval;
                fci_copy.param_count = 2;
                fci_copy.params = &call_params;

                const result = c.zend_call_function(&fci_copy, &fci_cache_copy);
                if (result != c.SUCCESS) {
                    return 0; // Error
                }
